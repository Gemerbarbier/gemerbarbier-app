 ...
 * Synchronously checks sessionStorage and redirects to /admin-dashboard/login if not authenticated.
...
const RequireAdminAuth = ({ children }: { children: React.ReactNode }) => {
  ...
  if (!isLoggedIn) {
    return <Navigate to="/admin-dashboard/login" replace state={{ from: location }} />;
  }
