...
const AdminDashboard = () => {
  ...
  type TabKey = "reservations" | "calendar" | "slots" | "stats";
  const [activeTab, setActiveTab] = useState<TabKey>(() => {
    const fromUrl = searchParams.get("tab") ?? "";
    const known: string[] = ["reservations", "calendar", "slots", "stats"];
    // A bookmarked ?tab=emails points at a tab that no longer exists — fall back rather than
    // rendering an empty page.
    return known.includes(fromUrl) ? (fromUrl as TabKey) : "reservations";
  });
