...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    for (const row of rows as EmailQueueRow[]) {
      ...
      try {
        const { subject, html, text } = renderTemplate(row.template_name, row.payload);
        const result = await sendViaResend(RESEND_API_KEY, row.recipient_email, subject, html, text);
